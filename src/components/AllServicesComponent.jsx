...
const AllServicesComponent = ({
  ...
  const filteredAndSearchedProviders = filteredProviders.filter((provider) => {
    ...
    if (selectedState === "HP" && selectedTehsil) {
      const tehsilLower = selectedTehsil.toLowerCase();
      matchesTehsil =
        (provider.tehsil || "").toLowerCase().includes(tehsilLower) ||
        (provider.address || "").toLowerCase().includes(tehsilLower);
    }
  ...
  useEffect(() => {
    // Only enable tehsil feature for Himachal Pradesh
    if (selectedState !== "HP") {
      if (selectedTehsil) setSelectedTehsil("");
      if (availableTehsils.length) setAvailableTehsils([]);
      return;
    }
    // When in HP and a city is selected, load tehsils for that city (if exists)
    if (selectedCity) {
      // Normalize key (API may return 'Lahaul and Spiti', map manually if needed)
      const normalized =
