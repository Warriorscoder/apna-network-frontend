...
// const Dialoguebox = ({ data, isOpen, onClose, allreviews}) => {
...
import {
  User, Star, MapPin, Clock, Award, Phone, MessageSquare,
  ChevronRight, Calendar, ArrowLeft, X, Edit,
} from "lucide-react";
import { toast } from "react-toastify";
import axios from "axios";
import { useAuth } from "@/app/context/Authcontext";
...
const Dialoguebox = ({ data, isOpen, onClose, allreviews, providerId, serviceId }) => {
  const [showReviews, setShowReviews] = useState(false);
  const [isSending, setIsSending] = useState(false);
  const { user } = useAuth();
  const [feedbackModalOpen, setFeedbackModalOpen] = useState(false);
  ...
  const totalReviews = safeAllReviews.length;

  // ✅ MODIFIED: Calculate average rating. If no reviews, rating is 0.
  const averageRating =
    totalReviews > 0
      ? (
  ...
  const ratingDistribution = [5, 4, 3, 2, 1].map((rating) => ({
    stars: rating,
    count: safeAllReviews.filter((review) => review.stars === rating).length,
    percentage:
      totalReviews > 0
  ...
  const handleOpenFeedbackModal = () => {
    ...
    if (user.role === 'provider') {
      toast.error("Service Providers cannot submit reviews.");
      return; // Stop the function and just show the toast
    }
        ...
        <div className="p-3 sm:p-6">
          {showReviews ? (
            ...
            <>
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  <button onClick={handleBackToProfile} className="p-1.5 sm:p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
                    <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600" />
                  </button>
                  <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                    <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "#695aa6" }}>
                      <User className="w-4 h-4 sm:w-6 sm:h-6 text-white" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h2 className="text-base sm:text-xl font-bold text-gray-800 truncate">{data.name} - Reviews</h2>
                      <div className="flex items-center space-x-1 sm:space-x-2">
                        <Star className="w-3 h-3 sm:w-4 sm:h-4 fill-yellow-400 text-yellow-400" />
                        <span className="text-xs sm:text-sm text-gray-600 font-medium">{averageRating} ({totalReviews} reviews)</span>
                      </div>
                    </div>
                  </div>
                </div>
                <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-1.5 sm:p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
                  <X className="w-5 h-5 sm:w-6 sm:h-6" />
                </button>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
                <div className="lg:col-span-1 order-1 lg:order-1">
                  <div className="bg-gray-50 rounded-lg p-4 sm:p-6 lg:sticky lg:top-0">
                    <div className="text-center mb-4 sm:mb-6">
                      <div className="text-2xl sm:text-4xl font-bold text-gray-800 mb-1 sm:mb-2">{averageRating}</div>
                      {renderStars(Math.round(averageRating), "w-4 h-4 sm:w-6 sm:h-6")}
                      <p className="text-xs sm:text-sm text-gray-600 mt-1 sm:mt-2">Based on {totalReviews} reviews</p>
                    </div>
                    <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                      <h4 className="font-semibold text-gray-800 mb-2 sm:mb-3 text-sm sm:text-base">Rating Distribution</h4>
                      {ratingDistribution.map((rating) => (
                        <div key={rating.stars} className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm">
                          <span className="w-2 sm:w-3 text-gray-600 text-xs sm:text-sm">{rating.stars}</span>
                          <Star className="w-3 h-3 sm:w-4 sm:h-4 fill-yellow-400 text-yellow-400 flex-shrink-0" />
                          <div className="flex-1 bg-gray-200 rounded-full h-1.5 sm:h-2">
                            <div className="bg-yellow-400 h-1.5 sm:h-2 rounded-full transition-all" style={{ width: `${rating.percentage}%` }}></div>
                          </div>
                          <span className="w-6 sm:w-8 text-gray-600 text-right text-xs sm:text-sm">{rating.count}</span>
                        </div>
                      ))}
                    </div>
                    <div className="border-t border-gray-200 pt-4">
                      ...
                      <p className="text-xs text-gray-500 text-center mt-2">Help others by sharing your experience</p>
                    </div>
                  </div>
                </div>
                <div className="lg:col-span-3 order-2 lg:order-2">
                  <div className="mb-3 sm:mb-4 lg:hidden">
                    <h3 className="text-base font-semibold text-gray-800">Customer Reviews ({totalReviews})</h3>
                  </div>
                  <div className="space-y-3 sm:space-y-4 max-h-[40vh] lg:max-h-[60vh] overflow-y-auto pr-1 sm:pr-2">
                    {totalReviews > 0 ? (
                      safeAllReviews.map((review) => (
                        <div key={review._id} className="border border-gray-200 rounded-lg p-3 sm:p-6 hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between mb-3 sm:mb-4">
                            <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-[#695aa6] to-[#5a4d8a] rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            <div className="flex items-center space-x-1 text-gray-500 flex-shrink-0">
                              <Calendar className="w-3 h-3 sm:w-4 sm:h-4" />
                              <span className="text-xs sm:text-sm">{formatDate(review.date)}</span>
                            </div>
                          </div>
                          <p className="text-gray-700 leading-relaxed mb-3 sm:mb-4 text-sm sm:text-base">{review.comment}</p>
                          <div className="flex items-center justify-end pt-2 sm:pt-3 border-t border-gray-100">
                            <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-green-100 text-green-700 rounded-full text-xs">Verified</span>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="text-center py-8 sm:py-12 text-gray-500">
                        <MessageSquare className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 text-gray-300" />
                        <h3 className="text-base sm:text-lg font-medium mb-1 sm:mb-2">No reviews yet</h3>
                        <p className="text-xs sm:text-sm">Be the first to leave a review for {data.name}!</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <>
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "#695aa6" }}>
                    <User className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <h2 className="text-lg sm:text-2xl font-bold text-gray-800 truncate">{data.name}</h2>
                    <div className="flex items-center space-x-2 sm:space-x-4 mt-1">
                      <div className="flex items-center space-x-1 sm:space-x-2">
                        <Star className="w-4 h-4 sm:w-5 sm:h-5 fill-yellow-400 text-yellow-400" />
                        <span className="text-gray-600 font-medium text-sm sm:text-base">{averageRating}</span>
                      </div>
                      <button onClick={() => setShowReviews(true)} className="flex items-center space-x-1 text-[#695aa6] hover:text-[#5a4d8a] transition-colors">
                        <MessageSquare className="w-3 h-3 sm:w-4 sm:h-4" />
                        <span className="text-xs sm:text-sm font-medium">{totalReviews} Reviews</span>
                        <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4" />
                      </button>
                    </div>
                  </div>
              ...
              <div className="mb-4 sm:mb-6">
                <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Service Details</h3>
                <div className="mb-3">
                  <h4 className="text-sm sm:text-base font-semibold text-gray-800 mb-1">Service Title</h4>
                  <p className="text-gray-700 text-sm sm:text-base">{data?.title || "Professional service provider"}</p>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                <div>
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex items-start space-x-2">
                      <MapPin className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 mt-0.5 flex-shrink-0" />
                      <span className="text-gray-600 text-sm sm:text-base break-words">{data.address || "Address not provided"}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 flex-shrink-0" />
                      <span className="text-gray-600 text-sm sm:text-base">{data.availability}</span>
                    </div>
                  </div>
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Experience Details</h3>
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex items-center space-x-2">
                      <Award className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" style={{ color: "#695aa6" }} />
                      <span className="text-gray-600 text-sm sm:text-base">{`${data.experience} years` || "Experience not specified"}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-gray-600 capitalize px-2 py-1 rounded bg-gray-100 text-xs sm:text-sm">{data?.category || "Unknown category"}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="mb-4 sm:mb-6">
                <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3">Service Tags</h3>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {data?.tags?.length ? (
                    data.tags.map((tag, index) => (
                      <span key={index} className="text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm" style={{ backgroundColor: "#695aa6" }}>
                        {tag}
                      </span>
                    ))
                  ) : (
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                <button
                  ...
                  style={{ backgroundColor: "#695aa6" }}
                  onMouseEnter={(e) => { if (!isSending) e.target.style.backgroundColor = "#5a4a96"; }}
                  onMouseLeave={(e) => { if (!isSending) e.target.style.backgroundColor = "#695aa6"; }}
                >
                  {isSending ? "Loading..." : "Send Email"}
                </button>
              </div>
