...
// ✅ OPTIMIZATION: Moved static data outside the component to prevent re-creation on every render.
const menuItems = [
  { key: "dashboard", icon: LayoutDashboard, label: "Dashboard" },
  { key: "requests", icon: ListTodo, label: "Requests" },
  { key: "services", icon: Briefcase, label: "Services" },
...
// ✅ OPTIMIZATION: Wrapped the entire component in React.memo to prevent unnecessary re-renders.
const Sidebar = memo(({ activeView, setActiveView, sidebarOpen, setSidebarOpen, isMobile }) => {
  const { user } = useAuth();

  // ✅ OPTIMIZATION: Memoized handler functions to stabilize their references.
  const handleItemClick = useCallback((key) => {
    setActiveView(key);
    if (isMobile) {
      setSidebarOpen(false); // Also close sidebar on mobile navigation
    }
...
Sidebar.displayName = 'Sidebar'; // Good practice for debugging memoized components
