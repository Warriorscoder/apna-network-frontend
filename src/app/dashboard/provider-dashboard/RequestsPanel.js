...
// ✅ OPTIMIZATION: Wrapped RequestCard in React.memo to prevent unnecessary re-renders
const RequestCard = memo(({ req, onStatusChange }) => {
  return (
    <div className="bg-white/90 rounded-lg shadow-sm border border-gray-200 p-4 space-y-3">
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-2">
...
RequestCard.displayName = 'RequestCard'; // Good practice for debugging memoized components
...
const RequestsPanel = () => {
  ...
  // ✅ OPTIMIZATION 1: Changed useEffect dependency from `user` to `user.id` for stability.
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        setLoading(true);
  ...
  // ✅ OPTIMIZATION 2: Wrapped handleStatusChange in useCallback to stabilize its reference.
  ...
  }, []); // Empty dependency array is correct as it only uses the stable `setRequests` function.
                ...
                <tbody className="divide-y divide-gray-200">
                  {requests.map((req) => (
                    <tr key={req._id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {/* ✅ OPTIMIZATION 3: Using the helper function to avoid duplicated logic */}
                        <select
                          value={req.status}
                          onChange={(e) => handleStatusChange(req._id, e.target.value)}
                          className={`px-2 py-1 text-xs rounded font-semibold ${getStatusColorClasses(req.status)}`}
                        >
                          <option value="requested">Requested</option>
                          <option value="accepted">Accepted</option>
                          <option value="in_progress">In Progress</option>
