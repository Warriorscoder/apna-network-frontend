...
// ✅ This custom hook is already efficient, no changes needed.
const useClientGreeting = () => {
  const [greeting, setGreeting] = useState("Welcome");
  useEffect(() => {
    const hour = new Date().getHours();
...
// ✅ Constant defined outside the component to prevent re-creation on every render.
const navItems = [
  { key: "dashboard", icon: LayoutDashboard, label: "Dashboard" },
  { key: "services", icon: Briefcase, label: "Services" },
  { key: "requests", icon: ListTodo, label: "My Requests" },
...
export default function UserDashboard() {
  ...
  const router = useRouter();
  const greeting = useClientGreeting();
  // ✅ OPTIMIZATION: Use a ref for the scroll position to avoid re-renders and effect loops.
  const lastScrollYRef = useRef(0);

  // Auth check useEffect (correct as is)
  useEffect(() => {
    if (user?.role === 'not logged in') {
      router.push('/login');
    }
  ...
  // Screen size checker useEffect (correct as is)
  useEffect(() => {
    const checkScreenSize = () => {
      const mobile = window.innerWidth < 1024;
      setIsMobile(mobile);
  ...
  // ✅ OPTIMIZATION 1: Fixed the inefficient scroll handler.
  useEffect(() => {
    if (!isMobile) return;
    ...
    window.addEventListener("scroll", handleScroll, { passive: true });
    return () => window.removeEventListener("scroll", handleScroll);
  }, [isMobile]); // This effect now only depends on `isMobile`.

  // ✅ OPTIMIZATION 2: Memoize handler functions to stabilize their references.
  ...
  // A memoized version of setActiveView to pass to children.
  const memoizedSetActiveView = useCallback(setActiveView, []);

  // ✅ OPTIMIZATION 3: Memoize the rendered content to prevent re-rendering the active panel unnecessarily.
  const contentPanel = useMemo(() => {
    switch (activeView) {
      case "dashboard":
        return <DashboardPanel setActiveView={memoizedSetActiveView} />;
      case "services":
        return <ServicesPanel />;
      case "requests":
        return <RequestsPanel />;
  ...
  }, [activeView, memoizedSetActiveView]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#a395d4] via-[#b8a7e8] to-[#8b7cc8] flex flex-col">
      ...
      {isMobile && !sidebarOpen && (
        <button
          onClick={handleSidebarToggle}
          className="lg:hidden fixed top-20 left-3 z-40 p-2.5 rounded-lg transition-opacity duration-300"
          style={{
            opacity: showFloatingMenu ? 0.8 : 0,
            pointerEvents: showFloatingMenu ? "auto" : "none",
            backgroundColor: "rgba(255, 255, 255, 0.2)",
      ...
      {isMobile && sidebarOpen && (
        <div onClick={() => setSidebarOpen(false)} className="fixed inset-0 bg-black/50 z-30"/>
      )}

      <main className="flex flex-1 pt-16 lg:pt-20 relative">
        {/* Sidebar */}
        <aside
          className={`fixed lg:sticky top-16 lg:top-20 left-0 h-[calc(100vh-4rem)] lg:h-[calc(100vh-5rem)] 
            ${sidebarOpen ? "translate-x-0 w-64" : "-translate-x-full lg:translate-x-0 lg:w-20"}
            bg-[rgba(60,50,100,0.95)] backdrop-blur-md text-white transition-all duration-300 ease-in-out z-40 lg:z-30 flex flex-col border-r border-white/10`}
        >
          <div className="p-4 border-b border-white/10 flex items-center justify-between lg:justify-center">
            ...
            {sidebarOpen && (
              <button className="flex items-center justify-center w-10 h-10 rounded-lg hover:bg-white/10 transition-colors lg:hidden" onClick={handleSidebarToggle} aria-label="Close sidebar">
                <X className="w-6 h-6" />
              </button>
            )}
            {!isMobile && (
              <button className="hidden lg:flex items-center justify-center w-10 h-10 rounded-lg hover:bg-white/10 transition-colors" onClick={handleSidebarToggle} aria-label={sidebarOpen ? "Collapse sidebar" : "Expand sidebar"}>
                <Menu className="w-6 h-6" />
              </button>
            )}
          </div>
          ...
          <nav className="flex-1 p-4">
            <ul className="space-y-2">
              {navItems.map(({ key, icon: Icon, label }) => (
                <li key={key}>
                  <button onClick={() => handleNavItemClick(key)}
                    className={`w-full flex items-center gap-4 p-3 rounded-lg transition-all duration-200 group ${activeView === key ? "bg-white/20 text-white shadow-lg" : "hover:bg-white/10 text-white/80 hover:text-white"} ${!sidebarOpen ? "lg:justify-center" : ""}`}
                    title={!sidebarOpen ? label : ""}
                  >
                    <Icon className={`w-6 h-6 flex-shrink-0 ${activeView === key ? "text-white" : "text-white/80 group-hover:text-white"}`} />
                    <span className={`font-medium transition-opacity duration-300 ${sidebarOpen ? "opacity-100" : "lg:opacity-0 lg:sr-only"}`}>
                      {label}
                    </span>
                  </button>
                </li>
        ...
        <section className={`flex-1 transition-all duration-300 ${isMobile ? "w-full" : ""}`}>
          {activeView === "dashboard" && (
            <div className="relative px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
              <div className="max-w-7xl mx-auto text-center">
                ...
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-3" style={{ textShadow: "0 4px 24px rgba(60,50,100,0.65), 0 2px 4px rgba(0,0,0,0.3)" }} suppressHydrationWarning={true}>
                  {greeting}, {user?.name || "Guest"}! 👋
                </h1>
                <p className="text-base sm:text-lg text-white/90 max-w-3xl mx-auto leading-relaxed" style={{ textShadow: "0 2px 12px rgba(60,50,100,0.45), 0 1px 2px rgba(0,0,0,0.3)" }}>
                  Connect with trusted local service providers in your community. From home repairs to personal services, find reliable professionals near you.
                </p>
              </div>
            </div>
          )}
          ...
          <div className={`px-4 sm:px-6 lg:px-8 pb-8 ${activeView !== "dashboard" ? "pt-8" : ""}`}>
            <div className="max-w-7xl mx-auto">{contentPanel}</div>
          </div>
        </section>
      </main>
    </div>
