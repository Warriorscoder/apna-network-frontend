...
const DeleteWarningModal = ({ isOpen, onClose, onConfirm, requestData }) => {
  if (!isOpen) return null;
  // This component's logic is fine, no changes needed here.
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              <div className="flex items-center space-x-2 text-sm">
                <Calendar className="w-4 h-4 text-gray-500" />
                <span className="font-medium">Date:</span>
                <span>{new Date(requestData?.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
          <div className="flex space-x-3">
...
// ✅ Wrapped RequestCard in React.memo to prevent unnecessary re-renders
const RequestCard = memo(({ req, onFeedback, onDelete }) => {
  const canDelete = req.status === "requested" || req.status === "cancelled";
  ...
  return (
    <div className="bg-white/90 rounded-lg shadow-sm border border-gray-200 p-4 space-y-3">
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center space-x-2">
          <span className={`px-2.5 py-1 text-xs rounded-full font-semibold border ${getStatusColorClasses(req.status)}`}>
            {req.status || "Pending"}
          </span>
          {canDelete && (
            <button
      ...
      <div className="pl-7 space-y-2 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          ...
          <span>
            Provider:{" "}
            <span className="font-medium text-gray-800">{req.provider_id?.name || "Unknown"}</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-gray-500" />
...
RequestCard.displayName = 'RequestCard'; // Good practice for memo components
...
export default function RequestsPanel() {
  ...
  // ✅ OPTIMIZATION 1: Corrected useEffect dependencies
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        setLoading(true);
  ...
  }, [user?.id]); // This now correctly depends only on the user's ID
  ...
  // ✅ OPTIMIZATION 2: Wrap prop functions in useCallback
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full bg-white/70 rounded-xl shadow-lg p-4 sm:p-6 border border-white/30 backdrop-blur-sm hover:shadow-xl transition-all">
        <h2 className="text-xl sm:text-2xl font-semibold mb-4 text-[#695aa6] flex items-center">
          <ListTodo className="w-5 h-5 sm:w-6 sm:h-6 mr-2" /> My Service Requests
          <span className="ml-auto bg-[#695aa6] text-white text-xs sm:text-sm px-2 py-1 rounded-full">
            {requests.length}
          </span>
        </h2>
          ...
          <div>
            <div className="space-y-3 lg:hidden">
              {requests.map((req) => (
                <RequestCard
                  key={req._id}
                  req={req}
                  onFeedback={openFeedbackModal} // Pass memoized handler
                  onDelete={openDeleteModal} // Pass memoized handler
                />
              ))}
            </div>
            ...
            <div className="hidden lg:block overflow-x-auto">
              <table className="min-w-full bg-white/90 rounded-lg shadow-sm text-sm text-gray-700">
                ...
                <tbody className="divide-y divide-gray-200">
                  {requests.map((req) => {
                    // ✅ OPTIMIZATION 3: Cleaned up duplicated logic
                    const canDelete = req.status === "requested" || req.status === "cancelled";
                    return (
                      <tr key={req._id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`px-2 py-1 text-xs rounded-full font-semibold ${getStatusColorClasses(req.status)}`}>
                            {req.status || "Pending"}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center space-x-2">
                            {req.status === "completed" && (
                              <button onClick={() => openFeedbackModal(req)} className="px-3 py-1.5 bg-[#695aa6] text-white rounded text-xs hover:bg-[#5a4d8a] transition">
                                Give Feedback
                              </button>
                            )}
                            {canDelete && (
